...
token = "OTQ2MzM4MTcyODY5MTQ4Njcz.YhdQGg.Dk9ShjpdPwjPyhNc8Xf7PX4aaXo"
...
async def on_connect():
    print(f"กำลังเปิดบอท ... : {bot.user}")
    time.sleep(1.0)
    print("〇 เปิดสำเร็จ 〇")
...
async def sms(ctx, phone, amount:int):
    ...
    if (amount < 151):
      ...
      embes = discord.Embed(title="𝗖𝗔𝗧 𝗖𝗢𝗠𝗠𝗨𝗡𝗜𝗧𝗬", description="Bot By : 𝗖-𝗖𝗔𝗧",color=0xff4612)
      embes.add_field(name="กำลังยิงไปที่เบอร์ ... ",value=phone)
      ima = "https://c.tenor.com/53exrMkY1I8AAAAd/anime-set-ready-go.gif"
      ...
      BBot(phone,amount)
    	...
    	await ctx.channel.send("😡ยิงไม่เกิน 151 ไอสัสบอทมันยิ่งกากยุ😡")
...
@bot.command()
async def help(ctx):
	emBed = discord.Embed(title="𝗖𝗔𝗧 𝗖𝗢𝗠𝗠𝗨𝗡𝗜𝗧𝗬",description="𝐇𝐎𝐖 𝐓𝐎 ?",color=0xff4612)
	emBed.add_field(name="𝐂𝐨𝐦𝐦𝐚𝐧𝐝 |",value=".sms [เบอร์] [จำนวน]")
